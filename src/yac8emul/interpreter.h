...
public:
  ...
  enum class instruction : std::uint8_t {
    ...
    // 2 -> Vx = Vx AND Vy
    // 3 -> Vx = Vx XOR Vy
    // 4 -> Vx += Vy, If the result is greater than 8 bits (i.e., > 255,) VF is
    // set to 1, otherwise 0. Only the lowest 8 bits of the result are kept, and
    // stored in Vx.
    // 5 -> Vx = Vx - Vy, If Vx > Vy, then VF is set to 1, otherwise 0. Then Vy
    // is subtracted from Vx, and the results stored in Vx.
    // 6 -> Vx >>= 1, if LSB is 1, VF is set to 1.
    // 7 -> Vx = Vy - Vx, If Vy > Vx, then VF is set to 1, otherwise 0. Then Vx
    // is subtracted from Vy, and the results stored in Vx.
    // E -> Vx <<= 1. If the most-significant bit of Vx is 1, then VF is set to
  ...
  cpu()
      : pc(0x200), RAM(), registers(), sp(0), stack(), frame_buffer(), i(0),
        random_state(1234), dt(0), st(0) {
...
private:
  ...
  std::array<std::array<std::uint8_t, 32 / sizeof(std::uint8_t)>,
             64 / sizeof(std::uint8_t)>
      frame_buffer;
